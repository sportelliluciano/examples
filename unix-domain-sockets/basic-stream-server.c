 ...
 * 
 * This server uses SOCK_STREAM protocol for sending and receiving a byte stream
...
int main() {
    ...
    int listener_socket = socket(PF_UNIX, SOCK_STREAM, 0);
    ...
    while (1) {
        ...
        while ((len = recv(data_socket, buffer, sizeof(buffer) - 1, 0)) > 0) {
            ...
            printf("%s\n", buffer);
            printf("--------------------------\n");
            // Echo data back. Since this is a stream socket the data may be
            // partially sent, we need to iterate to ensure the whole buffer
            // is sent.
            int data_sent = 0;
            while (data_sent < len) {
                int sent = send(data_socket, buffer + data_sent, len - data_sent, 0);
